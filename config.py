...
def get_prompts(city, day, maxtemp, lang):
    ...
    {{"contents":
     [
         {{"role": "user",
        "parts":[
                {{"text": "Give me five activities (each on line starting with -) (in country code: '{lang}' language) what to do in {city} (or home) if it is {lang}:'{day}' and max. temperature is: {maxtemp} ?"}}
            ]
         }}
     ]
    }}
